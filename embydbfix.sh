#!/bin/bash

# =========================================================================================
# 🎯 通用 Emby 数据库修复脚本 (v1.0)
#  
# 描述: 修复因全文搜索 (fts) 索引损坏导致的 Emby 数据库启动失败问题。
# 兼容性: 适用于 GNU/Linux 和 macOS/BSD 系统。
# 
# 用法: 
#   ./emby_db_repair.sh --path /path/to/emby/config
#
# -----------------------------------------------------------------------------------------
# ⚠️ 警告: 
# 1. 运行前【必须】手动停止 Emby 服务/Docker 容器。
# 2. 建议使用 'sudo' 运行以确保 chown/chmod 操作成功。
# =========================================================================================

# 默认变量
EMBY_DIR="/vol1/1000/docker/emby/config" #这是我飞牛emby数据库的地址
DB_FILENAME="library.db"
TABLE_PATTERN='fts_search9(_config|_content|_data|_docsize|_idx)?'

# -----------------------------------------------------------------------------------------
# ⚙️ 解析命令行参数
# -----------------------------------------------------------------------------------------

while [[ $# -gt 0 ]]; do
    case "$1" in
        --path)
            if [ -n "$2" ] && [ "${2:0:1}" != "-" ]; then
                EMBY_DIR="$2"
                shift 2
            else
                echo "❌ 错误: --path 缺少参数 (Emby 配置目录路径)." >&2
                exit 1
            fi
            ;;
        *)
            echo "❌ 错误: 未知参数 '$1'" >&2
            echo "用法: ./emby_db_repair.sh --path /path/to/emby/config" >&2
            exit 1
            ;;
    esac
done

# -----------------------------------------------------------------------------------------
# 检查与路径确认
# -----------------------------------------------------------------------------------------

if [ -z "$EMBY_DIR" ]; then
    echo "❌ 致命错误: 请使用 --path 参数指定 Emby 的配置根目录。" >&2
    exit 1
fi

# 确保路径以 /data 结尾的，我们取其父目录作为 EMBY_DIR
# 例如：如果用户输入 /path/to/emby/data，则将 EMBY_DIR 设为 /path/to/emby
if [[ "$EMBY_DIR" == *"/data" ]]; then
    EMBY_DIR=$(dirname "$EMBY_DIR")
fi

# 确保目标数据库文件存在
if [ ! -f "$EMBY_DIR/data/$DB_FILENAME" ]; then
    echo "❌ 致命错误: 在路径 '$EMBY_DIR/data/$DB_FILENAME' 未找到数据库文件。" >&2
    exit 1
fi

# -----------------------------------------------------------------------------------------
# ⚙️ 兼容性检查与函数定义
# -----------------------------------------------------------------------------------------

# 检查 sqlite3 是否安装
if ! command -v sqlite3 &> /dev/null; then
    echo "❌ 错误: sqlite3 命令行工具未安装。请先安装 (例如: sudo apt install sqlite3)。" >&2
    exit 1
fi

# 检查 stat 命令类型，定义兼容函数
if stat --version &> /dev/null; then
    # GNU (Linux) 模式
    get_stat() { stat --format "$1" "$2"; }
    sed_fix() { sed -i '$s/^ROLLBACK; -- due to errors/COMMIT;/' "$1"; }
else
    # BSD (macOS/FreeBSD) 模式
    get_stat() { stat -f "$1" "$2"; }
    sed_fix() { sed -i '' '$s/^ROLLBACK; -- due to errors/COMMIT;/' "$1"; }
fi

# -----------------------------------------------------------------------------------------
# 🚀 核心修复流程
# -----------------------------------------------------------------------------------------

echo "--- 修复开始 ---"
echo "✅ 目标目录: $EMBY_DIR"

cd "$EMBY_DIR" || exit 1 # 切换目录失败则退出

BACKUP_SQL="data/backup_$(date +%Y%m%d%H%M%S).sql" 
CLEAN_SQL="data/clean.sql"
NEW_DB="data/library_new.db"
DB_PATH="data/$DB_FILENAME"

# 步骤 1: 检查ACL权限并发出警告
if ls -ld "$DB_PATH" | grep -q '+'; then
    echo "⚠️ 警告: 原始文件具有 ACL ('+') 权限。脚本将无法复制 ACL。如果重启失败，请手动修复权限。"
fi

# 步骤 2: 导出原始数据库
echo "--- 2. 导出原始数据库 ($DB_PATH) ---"
if ! sqlite3 "$DB_PATH" ".dump" > "$BACKUP_SQL"; then
    echo "❌ 致命错误: 数据库导出失败。服务可能未停止。" >&2
    exit 1
fi

# 步骤 3: 移除目标表 (fts_search9)
echo "--- 3. 从备份中移除损坏的表 ($TABLE_PATTERN) ---"
grep -v -E "$TABLE_PATTERN" "$BACKUP_SQL" > "$CLEAN_SQL"

# 步骤 4: 修正 SQL 事务
echo "--- 4. 修正 SQL 事务： ROLLBACK -> COMMIT ---"
sed_fix "$CLEAN_SQL"

# 步骤 5: 生成新数据库
echo "--- 5. 生成新的数据库 ($NEW_DB) ---"
if ! sqlite3 "$NEW_DB" < "$CLEAN_SQL"; then
    echo "❌ 致命错误: 从 SQL 文件导入数据失败。" >&2
    exit 1
fi

# 步骤 6: 获取并应用权限和所有权
echo "--- 6. 获取并应用原始数据库的权限和所有权 ---"
# 获取权限和所有权，优先 GNU 格式，失败则使用 BSD 格式
perm=$(get_stat '%a' "$DB_PATH" 2>/dev/null)
owner=$(get_stat '%U:%G' "$DB_PATH" 2>/dev/null)
if [ -z "$owner" ]; then
    owner=$(get_stat '%Su:%Sg' "$DB_PATH")
    perm=$(get_stat '%A' "$DB_PATH")
fi

if ! chmod "$perm" "$NEW_DB" || ! chown "$owner" "$NEW_DB"; then
    echo "❌ 严重警告: 权限/所有权应用失败。请确保您使用 'sudo' 运行脚本。" >&2
fi
echo "    -> 权限应用: $perm, 所有者应用: $owner"

# 步骤 7: 备份原始数据库并替换
echo "--- 7. 备份原始数据库并替换 ---"
mv "$DB_PATH" "$DB_PATH.bak"
[ -f "data/library.db-shm" ] && mv "data/library.db-shm" "data/library.db-shm.bak"
[ -f "data/library.db-wal" ] && mv "data/library.db-wal" "data/library.db-wal.bak"

mv "$NEW_DB" "$DB_PATH"

echo "--- ✅ 修复完成。请重启 Emby 服务。 ---"
